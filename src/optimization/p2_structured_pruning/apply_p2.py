...
PRUNE_AMOUNT = 0.20   # prune 20% of conv output channels
FINETUNE_EPOCHS = 2
LEARNING_RATE = 1e-4

BASELINE_MODEL_PATH = Path(r"D:\github\IOT_Project\models\baseline\best_model.pth")
P2_MODEL_PATH = Path(r"D:\github\IOT_Project\models\optimized\p2_structured_pruned_model.pth")

RESULTS_DIR = Path(r"D:\github\IOT_Project\results\optimization")
REPORT_PATH = RESULTS_DIR / "p2_structured_pruning_report.txt"
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    progress_bar = tqdm(loader, desc="P2 Fine-tuning", leave=True)
...
def evaluate_accuracy(model, loader, device, desc="Evaluating P2 accuracy"):
    model.eval()
    correct = 0
    total = 0
...
def benchmark_inference(model, loader, device):
    ...
    with torch.no_grad():
        for images, _ in tqdm(loader, desc="Benchmarking P2 inference", leave=True):
            images = images.to(device)
...
# ===============================
def main():
    print("✂️ APPLYING P2: STRUCTURED PRUNING")
    ...
    P2_MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
    ...
    parameters_to_prune = apply_structured_pruning(model, amount=PRUNE_AMOUNT)
    ...
    for epoch in range(FINETUNE_EPOCHS):
        print(f"\n📚 P2 Fine-tuning Epoch {epoch + 1}/{FINETUNE_EPOCHS}")
    ...
    test_acc = evaluate_accuracy(
        ...
        test_loader_cpu,
        device=EVAL_DEVICE,
        desc="Evaluating P2 structured accuracy"
    )
    ...
    report = (
        "=== P2 STRUCTURED PRUNING REPORT ===\n\n"
        f"Training device: {TRAIN_DEVICE}\n"
        f"Evaluation device: {EVAL_DEVICE}\n"
        f"Baseline model path: {BASELINE_MODEL_PATH}\n"
        f"P2 model path: {P2_MODEL_PATH}\n\n"
        f"Baseline model size (MB): {baseline_size_mb:.4f}\n"
        f"P2 model size (MB): {p2_size_mb:.4f}\n"
        f"Size reduction (MB): {baseline_size_mb - p2_size_mb:.4f}\n"
        f"Compression ratio: {baseline_size_mb / p2_size_mb:.4f}\n\n"
        f"Structured prune amount target: {PRUNE_AMOUNT:.2f}\n"
        ...
        f"P2 test accuracy: {test_acc:.6f}\n"
    ...
    print("\n==============================")
    print("✅ P2 STRUCTURED PRUNING COMPLETED")
    print("==============================")
    print(f"Baseline model size (MB): {baseline_size_mb:.4f}")
    print(f"P2 model size (MB): {p2_size_mb:.4f}")
    print(f"Measured sparsity: {sparsity:.6f}")
    print(f"P2 test accuracy: {test_acc:.6f}")
