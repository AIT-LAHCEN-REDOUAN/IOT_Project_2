...
P3_MODEL_PATH = Path(r"D:\github\IOT_Project\models\optimized\p3_magnitude_pruned_model.pth")

RESULTS_DIR = Path(r"D:\github\IOT_Project\results\optimization")
REPORT_PATH = RESULTS_DIR / "p3_magnitude_pruning_report.txt"
...
def apply_layerwise_magnitude_pruning(model, amount=0.30):
    parameters_to_prune = []
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    progress_bar = tqdm(loader, desc="P3 Fine-tuning", leave=True)
...
def evaluate_accuracy(model, loader, device, desc="Evaluating P3 accuracy"):
    model.eval()
    correct = 0
    total = 0
...
def benchmark_inference(model, loader, device):
    ...
    with torch.no_grad():
        for images, _ in tqdm(loader, desc="Benchmarking P3 inference", leave=True):
            images = images.to(device)
...
# ===============================
def main():
    print("✂️ APPLYING P3: MAGNITUDE PRUNING")
    ...
    # 2) Apply magnitude pruning layer-wise
    parameters_to_prune = apply_layerwise_magnitude_pruning(model, amount=PRUNE_AMOUNT)
    ...
    for epoch in range(FINETUNE_EPOCHS):
        print(f"\n📚 P3 Fine-tuning Epoch {epoch + 1}/{FINETUNE_EPOCHS}")
    ...
    test_acc = evaluate_accuracy(
        ...
        test_loader_cpu,
        device=EVAL_DEVICE,
        desc="Evaluating P3 magnitude accuracy"
    )
    ...
    report = (
        "=== P3 MAGNITUDE PRUNING REPORT ===\n\n"
        f"Training device: {TRAIN_DEVICE}\n"
        f"Evaluation device: {EVAL_DEVICE}\n"
        f"Baseline model path: {BASELINE_MODEL_PATH}\n"
        f"P3 model path: {P3_MODEL_PATH}\n\n"
        f"Baseline model size (MB): {baseline_size_mb:.4f}\n"
        f"P3 model size (MB): {p3_size_mb:.4f}\n"
        f"Size reduction (MB): {baseline_size_mb - p3_size_mb:.4f}\n"
        f"Compression ratio: {baseline_size_mb / p3_size_mb:.4f}\n\n"
        f"Magnitude prune amount target: {PRUNE_AMOUNT:.2f}\n"
        ...
        f"P3 test accuracy: {test_acc:.6f}\n"
    ...
    print("\n==============================")
    print("✅ P3 MAGNITUDE PRUNING COMPLETED")
    print("==============================")
    print(f"Baseline model size (MB): {baseline_size_mb:.4f}")
    print(f"P3 model size (MB): {p3_size_mb:.4f}")
    print(f"Measured sparsity: {sparsity:.6f}")
    print(f"P3 test accuracy: {test_acc:.6f}")
